...
def test_case():
    ...
    save_path = 'data/'+scheme+'/'+dim+'/TC1_test/'
...
def wavemaker(k, dim, H0, Ly, Lw, input_data):
    ...
    def phii_ex_expr(function,x,z,t):
        function.interpolate( cos(k*x[0])*(exp(k*z)+exp(-k*z))*(-Aa*sin(w*t)+Bb*cos(w*t))/w )
...
def set_time(Lx, res_x, Ly, res_y, H0, n_z, Tw):
    ...
    dz = H0/n_z
    dx = Lx/Nx
    dt = 0.5*(min(dx,dz)/pi)       # time step
    T0 = 0.0                     # Initial time
    Tend = 3*Tw                    # Final time 
    t = T0                  # Temporal variable 
    dt_save = dt          # saving time step
    return T0, t, dt, Tend, dt_save, Nx, Ny