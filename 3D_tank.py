...
if case=='TC1':
    ...
    T0, t, dt, Tend, dt_save, Nx, Ny = set_time(Lx, res_x, Ly, res_y, H0, n_z, Tw)
else:
    if case=='TC2': 
...
while t<=Tend+smallfac: #  while t<Tend-dt: Onno 15-04-2020
    ...
    if t_save-smallfac < t: # Onno 13-04-2020
        ...
        if case=='TC1': #--output h(x,t) and phi(x,H0,t) and phi(x,0,t) for TC1--#
            op_file_name = tt+'.txt'
        ...
        if case=='TC2': #--Output h(x,y,t) for TC2 and TC2b--#
            ...
            y_coarse = np.linspace(0,Ly,9)
            x_coarse = np.linspace(0,Lx,41)
            for ix in x_coarse:
                for iy in y_coarse:
                    h_file.write('%-25s %-25s %-25s\n' %(str(ix), str(iy), str(h_n0.at(ix,iy))))
            h_file.close()
        
        # for TCU: you can output the data you want below
        # if case=='TCU': 
